...
//					$("#FourthDigit").blur();
					...
					if (n.length == 4) {
						...
						clearAllBoxes();
						$("#message-to-display_reenter_pin").html(NOT_VALID_PIN);
						$("#alert-dialog_reenter_pin").foundation("open");
	...
	try {
		...
		var ss = new cordova.plugins.SecureStorage(
			...
			function (error) {
				...
				$("#message-to-display_reenter_pin").html(error);
				$("#alert-dialog_reenter_pin").foundation("open");
			},
			'my_app'
		);
		...
		ss.set(
			function (key) {
				...
				//if fingerprint is available then navigate to fingerprint set-up page
				function isAvailableSuccess(result) {
					localStorage.setItem("isFingerPrintAvail", true);
					$("#fingerprint-basic").foundation('open');
				}
				//if fingerprint is not available then navigate to quick-pin set-up success page
		...
		//@Sagar first set the message to display and then display the alert
		$("#message-to-display_reenter_pin").html("Error in storing pin on device, please check security settings on your device");
		$("#alert-dialog_reenter_pin").foundation("open");
	}
}
...
function onBackClick() {
    localStorage.setItem('userNavigatefrom', 'loginSuccesspage');
