...
function openFingerprint() {
	...
	function errorCallback(err) {
		if (!(err == "Cancelled")) {
			...
			$("#max-attempts-msg").html("Invalid authentication/Fingerprint max attempts reached</br>Try login with your pin, you will now be taken there");
...
function getLoggedInUserDetails() {
  ...
  console.log('quick-login : Success Callback customer '+JSON.stringify(data));
					...
					if (data && data.givenNames && data.surname) {
						...
						if (data.title) {
							$("#logged-in-user-details").html("Hi " + data.title + " " + data.givenNames + " " + data.surname + "</br>Welcome to Mercer Super</br>Swipe left to see</br>your balance");
						} else {
							$("#logged-in-user-details").html("Hi " + data.givenNames + " " + data.surname + "</br>Welcome to Mercer Super</br>Swipe left to see</br>your balance");
						}
  ...
  console.log('quick-login : error Callback customer '+JSON.stringify(error));
					$("#user-data-loading-img-container").css("display", "none");
					$("#logged-in-user-details").css("margin-top", "30px");
					if (error.status === 0) {
