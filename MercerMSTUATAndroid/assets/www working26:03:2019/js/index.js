...
var app = {
    ...
    // Check if secure storge is availble
        ...
        loadConfigValuesFromServer.then( function success(data){
            getConfigValues().then(configUrlSuccess(data,ss),fnFailure());
        }, function error(){
            getConfigValues().then(configUrlSuccess(null,ss),fnFailure())
        });
...
if(jsonObject != null) {
    ...
    if(jsonObject.UseNewEndpointsAndroid){
        if(jsonObject.NewEndpoints){
            requestURL = authorizationUrl+ "/as/authorization.oauth2?client_id=mercersuperapp&redirect_uri="+app_redirect_url+
