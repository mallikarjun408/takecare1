...
//https://pf-mst.001.staging.ping-nonprod.mercerenc.com/idp/startSLO.ping?TargetResource=https%3A%2F%2Fwww.mercerfinancialservices.com%2F/
	...
	if (checkBox.checked == true) {
		...
		if (isNetworkAvailable()) {
			...
			var requestURL = authorizationUrl+ "/as/authorization.oauth2?client_id=mercersuperapp&redirect_uri="+app_redirect_url+
		...
		$("#message-to-display-TnC").html(TndC_ERROR_MESSAGE);
...
function getTokenFailure(error) {
	...
	$("#overlay-div").css("display", "none");
	if (error.status === 0) {
		$("#message-to-display").html(TIMEOUT_MESSAGE);
	}else{
		$("#message-to-display").html(ERROR_MESSGE);
	} 
...
function areLoginFieldsValid() {
	...
	if (empNotextBox.value == '') {
		...
		$("#message-to-display").html(ENTER_PIN_TEXT);
		$("#alert-dialog").foundation("open");
		return false;
