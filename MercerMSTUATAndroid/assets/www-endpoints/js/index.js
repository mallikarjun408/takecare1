...
function onBrowserLoadStart(event) {
    ...
    if (urlStringValue.indexOf("http://mercersuperapp://") >= 0) {
      ...
      /*  if (localStorage.getItem("isNewLogin") == "true") {
        alert(localStorage.getItem("isNewLogin") == "true");
 ...
 var appCode;
       ...
       // var appCode = urlStringValue.substring(urlStringValue.lastIndexOf("code") + 5, urlStringValue.lastIndexOf("scope") - 1)
        ...
        $.ajax({
            type: "POST",
            url: "https://pf-mst.001.staging.ping-nonprod.mercerenc.com/as/token.oauth2",
            async: false,
            timeout: 2000,
            data: requestData,
            headers: {
...
console.log(data +"  data at success  "+ JSON.stringify(data));
