			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data && data.givenNames && data.surname) {
						...
						$("#logged-in-user-details").css("margin-top", "30px");
						$("#logged-in-user-details").html("There was an error loading the details.<br>Please close app and try again.<br>If the problem persists call us on<br><a href='tel:1800 652 525'>1800 652 525</a>");
					}
...
function getUserBalanceDetails() {
	...
	ss.get(
		...
		function (value) {
			...
			$.ajax({
				type: "GET",
				url: baseUrl+"/v1/customer/balance",
				async: false,
				timeout: 20000,
				headers: {
					'Content-Type': 'application/x-www-form-urlencoded',
				...
				success: function (data) {
					...
					if (data && data.balance) {
						...
						$("#user-balance-para").html("There was an error loading the details.<br>Please close app and try again.<br>If the problem persists call us on<br><a href='tel:1800 652 525'>1800 652 525</a>");
					}
				},
				error: function (error) {
