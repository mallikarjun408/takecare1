...
function loginButtoClicked() {
    ...
    /*if (areLoginFieldsValid()) {
     ...
     localStorage.setItem("LoginTnCAccepted","true");
    ...
    if (checkBox.checked == true) {
        ...
        if (isNetworkAvailable()) {        //checking if the instance of in app browser is active
...
function displayAlert(url) {
    ...
    if (stringURL.indexOf("mercersuperapp://") >= 0) {
...
function getTokenSuccess(data) {
    ...
    if (data.access_token && data.refresh_token) {
        ...
        $("#message-to-display").html("Something went wrong</br>Please try again later");
        $("#alert-dialog").foundation("open");
    }
...
function getTokenFailure(error) {
    ...
    if (error.responseText) {
        $("#message-to-display").html("Something went wrong</br>Error:" + error.responseText.faultstring);
    } else {
        $("#message-to-display").html("Something went wrong</br>Please try again later</br>Code:" + error.status + "</br>Status Text:" + error.statusText);
    }
